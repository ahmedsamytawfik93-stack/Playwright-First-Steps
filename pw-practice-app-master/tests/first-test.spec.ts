...
test.skip('Locator syntax rules', async ({page}) => {
    ...
    // by Combination
    await page.locator('input#inputEmail1.shape-rectangle');
...
test.skip('User facing locators', async ({page}) => {
    ...
    //await page.getByTestId('test-id-email').click();
...
test('Assertion examples', async ({page}) => {
    ...
    const basicFormSectionButton = page.locator('nb-card').filter({hasText: 'Basic form'}).getByRole('button');
    
    // General Assertions
    const value = 5
    expect(value).toEqual(5)

    const buttonText = await basicFormSectionButton.textContent();
    expect(buttonText).toEqual('Submit');
